...
| `datasets/holdout/` — **held out, never tuned against** | **OpenAI assigns output rights to the user**, same terms as images 11–20 above | **self** + **page** (terms) | Eleven pages generated with an OpenAI image model, prompted by the repository owner on 2026-09-18. 71 items, 9 sensitive-tier, across an Aadhaar card, a community-services ID card and nine everyday forms, bills, invoices and labels. Adds three things no other corpus here has: a **Devanagari** name on an image, a **second checksum-invalid Aadhaar** canary, and **two people on one page** (the shipping label). **Its purpose is a number nothing was fitted to.** |
...
and then wrote the recognizers while looking at them. `holdout/`'s 71 items
