...
| **self** | produced in this repository by Claude (via Claude Code) under human direction, so the provenance is known first-hand rather than checked |
...
| `datasets/holdout/` — **held out, never tuned against** | **OpenAI assigns output rights to the user**, same terms as images 11–20 above | **self** + **page** (terms) | Twenty-one pages generated with an OpenAI image model, prompted by the repository owner on 2026-09-18 and 2026-09-19. 118 items, 14 sensitive-tier, across an Aadhaar card, two ID cards, and eighteen everyday forms, bills, invoices, letters, receipts and labels. Adds three things no other corpus here has: a **Devanagari** name on an image, a **Devanagari** name, **three checksum-invalid Aadhaars** (one handwritten), **two people on one page**, and the only pages carrying both a barcode and a QR code. **Its purpose is a number nothing was fitted to.** |
...
not thresholds. Scoring it once with an already-chosen configuration is its
purpose; sweeping configurations over it and keeping the winner is
forbidden. The condition is the repository owner's, set when the corpus was
contributed, and it is permanent — it does not lapse when a deadline is
close or a number is disappointing. `_meta` carries `held_out: true`,
`redactor.datasets.refuse_if_tuning()` enforces it, `benchmark_ocr.py` calls
that enforcement, and `test_holdout_guard.py` pins the whole path.

**It is not independent evidence, and no held-out discipline can make it
so.** The repository owner generated it, so it carries the same standing as
`datasets/documents/`: a ceiling on familiar material. What holding it out
buys is narrower and still worth having — a score that no parameter here was
fitted to. Independent evidence remains `datasets/cheques/`, IndiaPII-Bench
and maskara.

**All three are fully synthetic**: no real person, document, account or
photograph. The two `documents/` halves together are the 77-item ground
truth behind the 95.8% core headline — which `VALIDATION.md` states as a
**ceiling on familiar material**, because this project drew those documents
and then wrote the recognizers while looking at them. `holdout/`'s 118 items
are counted separately and are not part of that headline.
...
| part | status | effect |
|---|---|---|
| images 01–10 | rendered by a script **Claude wrote**, from values **Claude chose** | the script is code and is MIT; the rendered output is machine-generated, so no copyright is asserted over the images themselves |
...
| the annotations, the curation, the collage split | **Claude-generated, human-reviewed** — a person decided what counts as PII and audited the labels | the human selection and curation is the part that carries this repository's licence |

The practical position: **the corpus is freely usable, and essentially all
of it is AI-generated.** No copyright is asserted over machine-generated
output; the MIT grant covers the code and the human-directed selection and
curation. That is a statement about our rights, not a restriction on
yours — anyone generating a similar image independently owes nothing.
